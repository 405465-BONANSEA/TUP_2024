...
public class EscobaMatchRound {
    ...
    private void solveSevenWinner() {
        ...
        if (humanSevens > appSevens) {
            ...
            LetterByLetterPrinter.println("El jugador humano gana el punto por tener más sietes.");
        } else if (appSevens > humanSevens) {
            roundPlayerApp.setRoundPoints(roundPlayerApp.getRoundPoints() + 1);
            if (appSevens == 4) {
                roundPlayerApp.setRoundPoints(roundPlayerApp.getRoundPoints() + 1);
            }
            LetterByLetterPrinter.println("El jugador de la aplicación gana el punto por tener más sietes.");
        }
    }
    ...
    private Boolean sevenOroCheck(List<Card> cards) {
        ...
        for(Card card : cards) {
            if (card.getCardSuit().equals(CardSuit.OROS) && card.getNumber().equals(7)) {
                return true;
            }
        }
        return false;
